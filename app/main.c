...
int PlayGame(int pNum, int isMulti) {
    ...
    Rect(0, 0, 128, 15, C_BLK); for(k=1; k<5; k++) Rect(k*25, 0, 1, 15, C_GRY);
    ...
    int score = 0; 
    unsigned int ticks = 5000; // Frame-Zeit: Start-Geschwindigkeit (niedrig = schnell/flüssig)
    int max_active = 2;        
    int min_speed = 2; 
    ...
    while (1) {
        ...
        if (score > 30) max_active = 4;
        if (score > 60) max_active = 5;
        
        
        // Geschwindigkeit erhöhen
        // Alle 5 Punkte wird ticks um 600 reduziert (bis min 2000)
        ...
        if (active < max_active && (rand()%100) < 40) { 
            for(i=0; i<MAX_OB; i++) if (!obs[i].active) {
                int l = rand()%5, free = 1, j;
                // Prüfen ob Lane frei ist
                for(j=0; j<MAX_OB; j++) if(obs[j].active && obs[j].lane==l && obs[j].y < 45) free=0;
                if (free) { 
                    obs[i].active = 1; 
                    obs[i].lane = l; 
                    obs[i].x = lx[l]; 
